#ifndef __INFOWEBSERVER_H__
#define __INFOWEBSERVER_H__



/**
    ------ ------ ------ ------ ------ ------ library includes ------ ------ ------ ------ ------ ------
*/
#include <Arduino.h>

#include <functional>
#include "FS.h"

//#include "IOTAppStory.h"
#include <ESPAsyncWebServer.h>               // https://github.com/me-no-dev/ESPAsyncWebServer

#include <Web\TabbedAsyncDecl.h>

#include <Web\InfoTabItem.h>
#include <Web\InfoTabDropItem.h>
#include <SimpleMap.h>


class InfoTabItem;
class InfoTabDropItem;



	/**
		------ ------ ------ ------ ------ ------ macros for debugging ------ ------ ------ ------ ------ ------ 
	*/

	


	/**
		------ ------ ------ ------ ------ ------ STRUCTURES ------ ------ ------ ------ ------ ------

	*/

	/**
		------ ------ ------ ------ ------ ------ PROGMEM ------ ------ ------ ------ ------ ------
	*/

//File: favicon.ico.gz, Size: 726
#define favicon_ico_gz_len 726
const uint8_t favicon_ico_gz[] PROGMEM = {
 
};

class Task;
class Scheduler;

class InfoWebServer : public AsyncWebServer {
private:
	AsyncWebSocket *pWS;
	unsigned long lastUpdate;
	Scheduler *pRunner;
public:
	/**
		------ ------ ------ ------ ------ ------ VARIABLES ------ ------ ------ ------ ------ ------
	*/



	/**
		------ ------ ------ ------ ------ ------ FUCNTION DEFINITIONS ------ ------ ------ ------ ------ ------
	*/

	InfoWebServer(uint16_t port); //(const char *compDate, const int modeButton);
	~InfoWebServer(); //(const char *compDate, const int modeButton);
	void setup();
	void addSensor(InfoTabItem* pSensorItem);

	void loop();

protected:
	Task *pTask;

	void onTimer();

	void loadConfig();

	void addTab(InfoTabItem* handler);

	//bool removeTab(InfoTabItem* handler);
	
private:
	InfoTabDropItem* pConfigSensorsTab;
		

	/**
		------ ------ ------ ------ ------ ------ FUNCTION DEFINITIONS ------ ------ ------ ------ ------ ------
	*/

private:
	SimpleMap<uint32_t, AsyncWebSocketClient *> *mapConnections;

	static const int MAX_TAB_ITEMS = 10;
	//LinkedList<InfoTabItem*> _pageHandlers;
	InfoTabItem* arrTabItems[MAX_TAB_ITEMS];
	int tabItemCount;

	//int countSensors;
	
public:
//	InfoWebPage& addMenuItem(const char *menuId, const char *description, bool defaultTab, PageBuildCallback handler, void *object);
protected:
	void onBuildPage(AsyncWebServerRequest *request);
	void onWsEvent(AsyncWebSocket * server, AsyncWebSocketClient * client, AwsEventType type, void * arg, uint8_t *data, size_t len);


	//void onServeBootstrap(AsyncWebServerRequest *request);
	//void onServePopper(AsyncWebServerRequest *request);
	//void onServeBootstrapmin(AsyncWebServerRequest *request);


	String buildPage();
	String buildStyleSection();

	String buildHeadSection();
	String buildTabNavigation();
	String buildTabContentSection();

//	String buildNavigationPanel();
	String buildContentSection();
	String buildScriptSection();

	
protected:
	static String buildSensorsContents(void *object);
	static String buildStatusContents(void *object);
	static String buildAdminContents(void *object);
	//static String buildAdminContents(void *object);
	uint32_t getSensorCount() {return pConfigSensorsTab->GetDropItemCount();};
	
	InfoTabItem	*GetSensorAt(uint32_t idx) { return pConfigSensorsTab->GetDropItemAt(idx);	};

private:
	void addLine(String& html, String line);
	void append(String& html, String line);

	String buildSpecificTabContent(InfoTabItem* pItem, bool isActive);

	String generateTabItem(InfoTabItem* pItem, bool isActive);
	String generateTabDropItem(InfoTabItem* pItem);
//	String addTabLink(String id, String description);
//	String addTabContents(String id, String contents);

	/**
		------ ------ ------ ------ ------ ------ AUXILIARY FUNCTIONS ------ ------ ------ ------ ------ ------
	*/

};

#endif //__INFOWEBSERVER_H__
